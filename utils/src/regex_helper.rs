...
pub trait RegexHelper {
    ...
    fn find_iter_str<'a, 'b: 'a>(&'a self, s: &'b str) -> impl Iterator<Item = &'a str> + 'a;
...
impl RegexHelper for regex::Regex {
        ...
        T: itertools::traits::HomogeneousTuple<Item = &'b str>,
    {
        self.captures(s)
            ...
            .enumerate()
            .map(|(i, c)| {
                c.unwrap_or_else(|| panic!("No match for group {i}"))
                    .as_str()
            })
            .collect_tuple()
...
mod tests {
    ...
    fn find_iter() {
        itertools::assert_equal(
            regex!(r"\d+").find_iter_str("123 456 789"),
            ["123", "456", "789"],
        );
        itertools::assert_equal(
            [123, 456, 789],
            regex!(r"\d+").find_iter_parse("123 456 789"),
        );
    }
    ...
    fn numbers() {
        assert_eq!(
            vec![123, 456, 789],
            extract_numbers("123 456 789").collect_vec()
        );
        assert_eq!(
            vec![-123, 456, -789],
            extract_numbers("-123   456 -789").collect_vec()
